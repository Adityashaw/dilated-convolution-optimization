...
// cuda kernel
__global__ void Convolute(int input_row_size, int input_col_size, int *input, int kernel_row_size, int kernel_col_size,
                          int *kernel, int output_row_size, int output_col_size, ull *output)
{
    ...
    for (int kernel_row = 0; kernel_row < kernel_row_size; kernel_row++)
    {
        for (int kernel_col = 0; kernel_col < kernel_col_size; kernel_col++)
        {
            ...
            temp_output += input[input_row * input_col_size + input_col] * kernel[kernel_row * kernel_col_size + kernel_col];
        }
    }
            output[start_cell] = temp_output;
}

void gpuThread(int input_row_size, int input_col_size, int *input, int kernel_row_size, int kernel_col_size,
               int *kernel, int output_row_size, int output_col_size, ull *output)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        quarter_inputs[i] =
            new int[quarter_input_row_sizes[i] *
        ...
        cudaMemcpyAsync(dev_input[i], quarter_inputs[i],
                        sizeof(int) * quarter_input_row_sizes[i] * quarter_input_col_sizes[i], cudaMemcpyHostToDevice);
        ...
        Convolute<<<numBlocks, threadsPerBlock>>>(
            quarter_input_row_sizes[i], quarter_input_col_sizes[i], dev_input[i], kernel_row_size, kernel_col_size,
            dev_kernel, quarter_output_row_sizes[i], quarter_output_col_sizes[i], dev_output[i]);
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        cudaMemcpy(quarter_output, dev_output[i],
                   sizeof(ull) * quarter_output_row_sizes[i] * quarter_output_col_sizes[i], cudaMemcpyDeviceToHost);
